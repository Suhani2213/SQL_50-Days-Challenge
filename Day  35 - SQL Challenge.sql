...
select request_at,
	...
	count(case when t.status in("cancelled_by_client", "cancelled_by_driver") then 1 else null end ) as cancelled_rides,
	round((count(case when t.status in ("cancelled_by_client", "cancelled_by_driver") then 1 else null end)/ count(1)) * 100, 2) as cancelled_rate
from Trips t 
